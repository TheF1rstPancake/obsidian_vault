...
point: >
  ...
  authority over any of them. Creating the team also requires a conscious
  bet that engineering will build a platform whose primary users are internal
  FDEs. In AI products that split is especially clear: core owns the harness;
  the FDE owns prompts, tools, and customer-specific deployment. Which of
  those you actually own tells you which flavor of FDE you are, and which
  failure mode you're exposed to.
...
This is customer-facing implementation plus core engineering plus the architect function, on one team. The other product and engineering teams still exist, and each pulls in a different direction. The customer-facing org wants the deal unstuck. Product wants the pattern to generalize. Engineering wants the platform clean. The customer wants their specific thing, today. The FDE is the only person in the room accountable for all four landing, and owns none of the systems that would let them negotiate any of the four down. The only lever they actually control is the workaround. So when those priorities conflict (and they usually do), the FDE falls back to writing another workaround and figuring out the rest later.
...
Creating an FDE team only works if you make a conscious decision first: engineering prioritizes building a platform and framework whose primary users are internal. Some customers may take advantage of the customization and extension points directly. Day to day, the people using those systems will mostly be your FDE team. If engineering won't fund that internal platform, you are not ready to hire FDEs. You are asking them to invent the kit and ship last-mile work on the same calendar.

Think of that platform as a Lego kit. The core engineering team designs and produces the bricks. The forward deployed engineers combine those bricks into solutions shaped to each customer's situation. The custom code composes on top of the platform rather than living inside it. When the FDE team sees the same pattern across customers, that pattern becomes a candidate for first-class inclusion in the platform, and the decision about whether to absorb it belongs to core engineering, not the FDE. This is, as I read it, more or less the Palantir model, and I think most companies citing it are reading it wrong.
...
That model only works if core engineering takes the internal-platform job seriously. If the kit doesn't have bricks, the FDE has nothing to combine. If core engineering won't make absorption decisions, patterns pile up in customer code forever. If nobody owns Frankenstein-prevention, every customer slowly ends up with a slightly different version of the product.
...
Everything above holds no matter what an FDE is shipping. More and more, that thing is an AI agent.

The core building block of an AI agent is the harness: how the model receives instructions, what tools it can call, how you configure it to take on new work, the evals and observability that tell you whether it's working, the deployment surface, the security boundary around all of it. Once that framework exists, the last mile for a specific customer request is mostly modifying prompts, wiring context, and creating the tools the harness doesn't ship. The surface-area split gets clean. Core owns the harness. The FDE owns prompts, tools, and the customer-specific deployment on top.

You can also choose not to build that flexibility: one agent configuration for everyone. In that case you don't need an FDE. The customization work isn't there to do.

Draw the ownership line in the wrong place and you can watch it do damage. Airtable scripting started in the browser. Because the code ran in the customer's own browser, it inherited the browser's reach: it could make web requests, reach a service on the customer's VPN, talk to something running on their laptop. People built real workflows on that. When scripting moved to automations so it could run on a schedule instead of a button click, it moved onto our AWS infrastructure, and that runtime change quietly deleted every use case that depended on reaching the customer's own network. The people most excited to automate those jobs got the least. That was a harness decision, runtime and security boundary, and it belonged to core engineering. No amount of FDE prompt-and-tool work at the deployment layer could route around it, because the FDE doesn't own the runtime. An FDE can add a tool. An FDE cannot change what the agent is fundamentally allowed to touch.

In AI products the division of labor is unusually clear. Harness is core. Prompt engineering and tools development is FDE. The three questions from the top of this piece still apply to what gets promoted into the harness, who maintains it, and what customer-shaped weirdness gets refused. The ownership line itself is not ambiguous.

Look at where your week goes, not at what your badge says.
...
If your organization is past the earliest stage and the honest answer to "who actually delivers value to customers" is "the four-person forward deployed team," that isn't a resourcing gap. It's a product and engineering problem wearing a resourcing costume. A small team is not supposed to be the linchpin between your product and your revenue. If it is, ask the harder question: why does customer acceptance of your product require a skunkworks team to get them over the line? Why isn't the product designed so the value and implementation path are self-evident enough that closing the deal doesn't depend on four people writing one-offs? Hiring more forward deployed engineers treats the symptom instead of that cause.

That points at an obligation running the other direction from how most job postings frame it. Product and engineering owe the forward deployed team a platform to build on top of, not as a courtesy but as a precondition. If core engineering won't build and maintain that platform (the Lego kit, the toolkits, the reusable pieces), the organization isn't ready to have forward deployed engineers. Skip that precondition and what you've actually hired is a team of core engineers wearing a customer-facing title, expected to build the internal platform, use it, and face the customer, all on a timeline set by whichever deal is closing this week. That's not a forward deployed engineering team. That's three jobs done badly by people you're paying for one.

What comes next is not a renamed version of the early model. As I understand the [Palantir setup](https://blog.palantir.com/dev-versus-delta-demystifying-engineering-roles-at-palantir-ad44c2a6e87), their FDEs are a customer-facing implementation team that uses toolkits engineering provides to unblock customers, working closely with core engineering to absorb the worth-absorbing patterns into the product over time. The embedding and the production code are downstream of that, not the substance of it. Different team. Different mandate. Different bar to clear.
...
Past the early stage, hiring FDEs into engineering to absorb one-offs collapses the split, skips the architect, and puts you back in the muddle. One concrete check, independent of headcount: who the team reports to. Report into engineering and they become the buffer for whatever product and engineering haven't prioritized. Report into go-to-market, with a real bridge into engineering's roadmap through the architect tier, and the job stays last-mile delivery.

An FDE org that is really a renamed SE team produces a consulting shop. The structural cause is usually that you tried to skip the architect. Engineering owns the kit. Implementation combines the blocks. The architect owns what the platform should learn next. Pretend those are one job and you get the muddle most current FDE roles are producing.
...
[^pm]: A stranger possibility, more speculative than the rest of this piece: the architect tier and product management might not stay separate roles for long. Forward deployed engineers are closest to what customers actually need and closest to what's technically possible; PMs are increasingly doing the same scoping and prioritization work from further away. If those two jobs collapse into one, the more interesting question isn't the org chart, it's what's left of product management as its own function once that happens. Possibly not much beyond prioritization, and possibly that prioritization ends up owned by an engineering leader or a standing group of executives rather than a dedicated product org. I don't have enough evidence to claim this, only enough to flag it.
